...
CREATE TABLE SYS_USER_EXTEND(
  ...
  UPD_ORG_CODE  VARCHAR2 (32),
  UPD_DATE  DATE,
CONSTRAINT SYS_USER_EXTEND PRIMARY KEY (USER_ID)
);
 COMMENT ON TABLE SYS_USER_EXTEND IS '系统管理-用户扩展信息表';
 COMMENT ON COLUMN SYS_USER_EXTEND.USER_ID IS  '主键';
COMMENT ON COLUMN SYS_USER_EXTEND.POLITICAL_STATUS IS  '政治面貌:团员、党员、其他党派人士、无党派';
COMMENT ON COLUMN SYS_USER_EXTEND.PARTY_TIME IS  '入党时间';
COMMENT ON COLUMN SYS_USER_EXTEND.MARITAL_STATUS IS  '婚姻状况:10-未婚；20-已婚；21-离婚；40-丧偶；';
COMMENT ON COLUMN SYS_USER_EXTEND.BLOOD_TYPE IS  '血型:A型、B型、AB型、O型、未知血型';
COMMENT ON COLUMN SYS_USER_EXTEND.ENGLISH_LEVEL IS  '英语等级';
COMMENT ON COLUMN SYS_USER_EXTEND.COMPUTER_LEVEL IS  '计算机等级';
COMMENT ON COLUMN SYS_USER_EXTEND.WORK_TIME IS  '参加工作年月';
COMMENT ON COLUMN SYS_USER_EXTEND.INDUCTION_TYPE IS  '入职类型:社会招录、资源性人才引进、系统内调入、系统外调入';
COMMENT ON COLUMN SYS_USER_EXTEND.EMPLOY_WAY IS  '用工方式:合同制员工
派遣制员工
实习员工';
COMMENT ON COLUMN SYS_USER_EXTEND.EMP_STATUS IS  '员工状态:在职在岗
在编不在岗的
离岗退养
转非
退休
死亡';
COMMENT ON COLUMN SYS_USER_EXTEND.JOIN_DATE IS  '入职日期';
COMMENT ON COLUMN SYS_USER_EXTEND.HEIGHT IS  '身高';
COMMENT ON COLUMN SYS_USER_EXTEND.WEIGHT IS  '体重';
COMMENT ON COLUMN SYS_USER_EXTEND.CHILD_NUM IS  '子女人数';
COMMENT ON COLUMN SYS_USER_EXTEND.HEALTH_STATE IS  '健康状况:健康
一般
慢性疾病
生理缺陷
残废
较弱';
COMMENT ON COLUMN SYS_USER_EXTEND.HIGHEST_EDU IS  '最高学历:中专、高中、大专、专科、本科、研究生（硕士）、研究生（博士）';
COMMENT ON COLUMN SYS_USER_EXTEND.HIGHEST_DEGREE IS  '最高学位:0-其他；1-名誉博士；
2-博士；
3-硕士；
4-学士；
9-未知。';
COMMENT ON COLUMN SYS_USER_EXTEND.CONTACT_ADDRESS IS  '通讯地址';
COMMENT ON COLUMN SYS_USER_EXTEND.POSTCODE_CONTACT IS  '通讯地址邮编';
COMMENT ON COLUMN SYS_USER_EXTEND.RESIDENTIAL_ADDRESS IS  '现居住地址';
COMMENT ON COLUMN SYS_USER_EXTEND.POSTCODE_RESIDENTIAL IS  '现居住地邮编';
COMMENT ON COLUMN SYS_USER_EXTEND.DOMICILE_ADDRESS IS  '户籍所在地';
COMMENT ON COLUMN SYS_USER_EXTEND.POSTCODE_DOMICILE IS  '户籍所在地邮编';
COMMENT ON COLUMN SYS_USER_EXTEND.HOUSEHOLD_REG IS  '户籍类型:城镇,非城镇';
COMMENT ON COLUMN SYS_USER_EXTEND.EMAIL IS  '电子邮箱';
COMMENT ON COLUMN SYS_USER_EXTEND.MOBILE_PHONE IS  '手机';
COMMENT ON COLUMN SYS_USER_EXTEND.HOME_TELPHONE IS  '家庭电话';
COMMENT ON COLUMN SYS_USER_EXTEND.EMERGENCY_CONTACT IS  '紧急联系人';
COMMENT ON COLUMN SYS_USER_EXTEND.EMERGENCY_CONTACT_PHONE IS  '紧急联系人手机';
COMMENT ON COLUMN SYS_USER_EXTEND.QQ IS  'QQ号';
COMMENT ON COLUMN SYS_USER_EXTEND.WECHAT IS  '微信号';
COMMENT ON COLUMN SYS_USER_EXTEND.REMARK IS  '备注';
COMMENT ON COLUMN SYS_USER_EXTEND.LEGAL_ORG IS  '法人机构编号';
COMMENT ON COLUMN SYS_USER_EXTEND.CRT_USER_CODE IS  '创建人';
COMMENT ON COLUMN SYS_USER_EXTEND.CRT_ORG_CODE IS  '创建机构';
COMMENT ON COLUMN SYS_USER_EXTEND.CRT_DATE IS  '创建日期';
COMMENT ON COLUMN SYS_USER_EXTEND.UPD_USER_CODE IS  '修改人';
COMMENT ON COLUMN SYS_USER_EXTEND.UPD_ORG_CODE IS  '修改机构';
COMMENT ON COLUMN SYS_USER_EXTEND.UPD_DATE IS  '修改日期';
...
CREATE TABLE SYS_MENU(
  ...
  UPD_ORG_CODE  VARCHAR2 (32),
  UPD_DATE  DATE,
CONSTRAINT SYS_MENU PRIMARY KEY (MENU_ID)
);
...
COMMENT ON COLUMN SYS_MENU.PARENT_MENU_ID IS  '所属父菜单';
