...
public class BeanUtil {
	 ...
	 * @param oldObj 被复制处理的对象
	 * @param newObj 得到放射后的对象
	 */
				...
				for(int j=0;j<method2.length;j++){
					...
					if(methodName2.startsWith("set")){
						if(methodFix2.equals(methodFix1)){
							...
							objs2[0]=method1[i].invoke(oldObj,objs1);//激活obj1的相应的get的方法，objs1数组存放调用该方法的参数,此例中没有参数，该数组的长度为0
							method2[j].invoke(newObj,objs2);//激活obj2的相应的set的方法，objs2数组存放调用该方法的参数
							continue;              
						}
					}
				}
